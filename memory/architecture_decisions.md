...
- The checkpoint also records whether `state.view` was `'preview'` or `'session'` at save time, and `resumeActiveSession()` restores that exact view. **First version of this always forced `state.view = 'session'` on resume** — user hit this directly: a reload while sitting on a preview screen (watching the exercise video) dropped them straight into the running timer, skipping the video for that occurrence entirely ("I see the timer before the video"). Fixed by saving `view` in the checkpoint and, just as importantly, moving the `saveActiveSession()` call in `startTimer()`'s transition branch to *after* `state.view` is reassigned for the new item — it was being called while `state.view` still held the previous item's view, so the checkpoint was one step stale even before `resumeActiveSession()` ignored it.
